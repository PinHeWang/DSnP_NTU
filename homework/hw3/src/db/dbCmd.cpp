...
DBAppendCmd::exec(const string& option)
{
   ...
   if(options.size() < 2) return CMD_EXEC_ERROR;
   vector<int> data;
   ...
   if(doRow){
      data.resize(dbtbl.nCols());
      DBRow r(data);
      dbtbl.addRow(r);
   }else{
      data.resize(dbtbl.nRows());
      dbtbl.addCol(data);
   }
...
{  
   ...
   if (options.size() < 1) return CmdExec::errorOption(CMD_OPT_MISSING, "");
      ...
      /*
      stringstream extraStr;
      for(int i=2; i<options.size(); i++) 
         extraStr << options[i] << " ";
      */
      //string str(options.begin(), options.end());
   ...
   int n0, n1;
   if(options.size() == 2) {
      if(myStr2Int(options[0], n0)) {
         if (!checkRowIdx(options[0], n0)) return CMD_EXEC_ERROR;
         if (!checkColIdx(options[1], n1)) return CMD_EXEC_ERROR;
         if(dbtbl[n0][n1] == INT_MAX) cout << "null";
         else cout << dbtbl[n0][n1];
      } else {
         if(myStrNCmp("-Row", options[0], 2) == 0) {
            if (!checkRowIdx(options[1], n1)) {return CMD_EXEC_ERROR;}
            else cout << dbtbl[n1];
         }else if(myStrNCmp("-Column", options[0], 2) == 0) {
            if (!checkColIdx(options[1], n1)) {return CMD_EXEC_ERROR;}
            else dbtbl.printCol(n1);
