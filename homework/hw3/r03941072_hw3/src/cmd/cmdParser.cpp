...
{
   // TODO...
   _dofile = new ifstream(dof.c_str()); //A pointer to the c-string representation of the string object's value.
   if (!(*_dofile).good()) {(_dofileStack.empty())? _dofile = 0: _dofile = _dofileStack.top(); return false;}
   if (_dofileStack.size() < 1024) {_dofileStack.push(_dofile); return true;}
   _dofile = 0;
   return false;  
}
...
CmdParser::closeDofile()
{
   ...
   if (_dofileStack.top() == _dofile)  _dofileStack.pop(); 
   delete _dofile;
...
CmdParser::parseCmd(string& option)
{
   ...
   if (!e) cerr << "Illegal command!! \"" << str << "\"";
   if (str.size() != cmd.size()) {
      size_t n = str.find_first_not_of(" ", cmd.size());
      if (n != string::npos) option = str.substr(n);
   }
   return e;
   //return NULL;
}
...
CmdParser::listCmd(const string& str)
{
   ...
   for(CmdMap::const_iterator i = _cmdMap.begin(); i != _cmdMap.end(); i++){
      if(cmd == "" || myStrNCmp(i->first, cmd, min(i->first.size(),  cmd.size())) == 0){
         container.push_back(i->first + i->second->getOptCmd());
         cmdCaptured = (CmdRegPair *)&(*i);
      }
   }

   if(container.size() == 1){
      if(container[0].size() != cmd.size() - 1){
         if ((size_t)(_readBufPtr - _readBuf) <= cmd.length()) {
            for (size_t cmdlen = cmd.length(); cmdlen < container[0].length(); cmdlen++)
               insertChar(container[0][cmdlen]);
            insertChar(' ');
      }else {
      ...
      for(int i=0; i<container.size(); i++){
         cout << setw(12) << left << container[i];
         countLine++;
         if(countLine % 5 == 0) cout << endl;
      }
      reprintCmd();
   }else {
      if(cmd.length() == cmdCaptured->first.size()) mybeep();
      else {
         cout << endl;
   ...
   //cout << "cmd size is " << cmd.length();
   //cout << "Container size is " << container.size();
   //mybeep();
...
CmdParser::getCmd(string cmd)
{
   CmdExec* e = 0;
   // TODO...
   CmdMap::const_iterator i;
   for (i = _cmdMap.begin(); i != _cmdMap.end(); ++i) {
      string mandCmd = cmd.substr(0, i->first.size());
      string optCmd;
      if (mandCmd.size() != cmd.size()) optCmd = cmd.substr(i->first.size());

      if (myStrNCmp(i->first, mandCmd, i->first.size()) == 0) {
         if (optCmd.size()) {
            string cmpOptCmd = i->second->getOptCmd();
            if (optCmd.size() > cmpOptCmd.size()) break;
            cmpOptCmd = cmpOptCmd.substr(0, optCmd.size());
            if (myStrNCmp(cmpOptCmd, optCmd, optCmd.size()) != 0 ) break;
         }
         return i->second;
      }
   }
