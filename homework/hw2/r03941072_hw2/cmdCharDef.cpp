   ...
   switch (ch) {
      ...
      case ESC_KEY: {
       char combo = mygetc(istr);
         ...
         if ((key == char(MOD_KEY_CAT1)) || (key == char(MOD_KEY_CAT2))) {
               char MODkey = mygetc(istr);
            if ((MODkey >= char(MOD_KEY_BEGIN)) && (MODkey <= char(MOD_KEY_END))){
               if (mygetc(istr) == MOD_KEY_DUMMY)
                  return returnCh(int(MODkey) + MOD_KEY_FLAG);
               else
